...
$(TYPEDEF)
"""
struct Scott2013Cond <: Function
    nx::Int
    np::Int
    etol::Float64
end
function (d::Scott2013Cond)(g::Vector{Float64}, u::Vector{Float64}, t::Float64, integrator)::Nothing
    for i=1:d.nx
...
$(TYPEDEF)
"""
struct Scott2013Affect <: Function
    np::Int
    nx::Int
end
function (d::Scott2013Affect)(integrator, idx::Int)::Nothing
...
$(TYPEDEF)
"""
struct Scott2013AffectNeg <: Function
    np::Int
    nx::Int
end
function (d::Scott2013AffectNeg)(integrator, idx::Int)::Nothing
...
function (d::Scott2013f{MC{N,T},F})(dx::Vector{Float64}, x::Vector{Float64},
...
The Scott2013 type integrator represents the relaxed pODE problem as a
...
$(TYPEDFIELDS)
"""
mutable struct Scott2013{F, N, T<:RelaxTag, PRB1<:AbstractODEProblem, PRB2<:AbstractODEProblem,
...
function Scott2013(d::ODERelaxProb; calculate_relax::Bool = true,
    ...
    scott2013cond! = Scott2013Cond(d.nx, np, event_soft_tol)
    scott2013affect! = Scott2013Affect(np, d.nx)
    scott2013affectneg! = Scott2013AffectNeg(np, d.nx)
    vector_callback = VectorContinuousCallback(scott2013cond!, scott2013affect!,
                                               2*d.nx; affect_neg! = scott2013affectneg!,
    ...
    f = Scott2013f(d.f, Z, d.nx, np, P, calculate_relax, calculate_subgradient,
    ...
    Scott2013(calculate_relax, calculate_subgradient, differentiable,
...
function DynamicBoundsBase.relax!(d::Scott2013{F, N, T, PRB1, PRB2, INT1, CB}) where {F, N, T<:RelaxTag,
    ...
    if d.integrator_state.new_decision_pnt
        ...
        if d.calculate_relax
            relax_ode_sol = solve(d.relax_ode_prob, d.relax_ode_integrator, callback = d.vector_callback)
        else
...
supports(::Scott2013, ::IntegratorName) = true
supports(::Scott2013, ::Gradient) = true
supports(::Scott2013, ::Subgradient) = true
supports(::Scott2013, ::Bound) = true
supports(::Scott2013, ::Relaxation) = true
supports(::Scott2013, ::IsNumeric) = true
supports(::Scott2013, ::IsSolutionSet) = true
supports(::Scott2013, ::TerminationStatus) = true
supports(::Scott2013, ::Value) = true
supports(::Scott2013, ::ParameterValue) = true

get(t::Scott2013, v::IntegratorName) = "Scott 2013 Integrator"
get(t::Scott2013, v::IsNumeric) = false
get(t::Scott2013, v::IsSolutionSet) = true
get(t::Scott2013, s::TerminationStatus) = t.termination_status

function getall!(out::Array{Float64,2}, t::Scott2013, v::Value)
    out .= t.local_problem_storage.pode_x
    return
end

function getall!(out::Vector{Array{Float64,2}}, t::Scott2013, g::Gradient{NOMINAL})
    for i in 1:t.np
...
function getall!(out::Vector{Array{Float64,2}}, t::Scott2013, g::Gradient{LOWER})
    if ~t.differentiable
...
function getall!(out::Vector{Array{Float64,2}}, t::Scott2013, g::Gradient{UPPER})
...
function getall!(out::Vector{Array{Float64,2}}, t::Scott2013, g::Subgradient{LOWER})
    for i in 1:t.np
        if t.evaluate_interval
            fill!(out[i], 0.0)
        else
...
function getall!(out::Vector{Array{Float64,2}}, t::Scott2013, g::Subgradient{UPPER})
    for i in 1:t.np
        if t.evaluate_interval
            fill!(out[i], 0.0)
        else
...
function getall!(out::Array{Float64,2}, t::Scott2013, v::Bound{LOWER})
    out .= t.relax_lo
    return
end

function getall!(out::Vector{Float64}, t::Scott2013, v::Bound{LOWER})
    out[:] = t.relax_lo[1,:]
    return
end

function getall!(out::Array{Float64,2}, t::Scott2013, v::Bound{UPPER})
    out .= t.relax_hi
    return
end

function getall!(out::Vector{Float64}, t::Scott2013, v::Bound{UPPER})
    out[:] = t.relax_hi[1,:]
    return
end

function getall!(out::Array{Float64,2}, t::Scott2013, v::Relaxation{LOWER})
    if t.evaluate_interval
...
function getall!(out::Vector{Float64}, t::Scott2013, v::Relaxation{LOWER})
    if t.evaluate_interval
        @inbounds for i in eachindex(out)
            out[i] = t.X[i].lo
        end
...
function getall!(out::Array{Float64,2}, t::Scott2013, v::Relaxation{UPPER})
    if t.evaluate_interval
        @inbounds for i in eachindex(out)
            out[i] = t.X[i].hi
        end
...
function getall!(out::Vector{Float64}, t::Scott2013, v::Relaxation{UPPER})
    if t.evaluate_interval
        @inbounds for i in eachindex(out)
            out[i] = t.X[i].hi
        end
...
function setall!(t::Scott2013, v::ParameterBound{LOWER}, value::Vector{Float64})
...
function setall!(t::Scott2013, v::ParameterBound{UPPER}, value::Vector{Float64})
    t.integrator_state.new_decision_box = true
    @inbounds for i in 1:t.np
        t.pU[i] = value[i]
    end
...
function setall!(t::Scott2013, v::ParameterValue, value::Vector{Float64})
...
function setall!(t::Scott2013, v::Bound{LOWER}, values::Array{Float64,2})
...
function setall!(t::Scott2013, v::Bound{LOWER}, values::Vector{Float64})
    if t.integrator_state.new_decision_box
        t.integrator_state.set_lower_state = true
    end
...
function setall!(t::Scott2013, v::Bound{UPPER}, values::Array{Float64,2})
    if t.integrator_state.new_decision_box
...
function setall!(t::Scott2013, v::Bound{UPPER}, values::Vector{Float64})
    if t.integrator_state.new_decision_box
        t.integrator_state.set_upper_state = true
    end
    @inbounds for i in 1:t.steps
